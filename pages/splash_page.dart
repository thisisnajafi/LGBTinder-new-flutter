...
class _SplashPageState extends ConsumerState<SplashPage> {
  ...
  Future<void> _initializeApp() async {
    // Short delay so splash is visible; avoid long waits that cause ANR
    ...
    try {
      ...
      // Timeout: secure storage can be very slow on some devices (5–7s); avoid ANR
      final bool isAuthenticated = await tokenStorage.isAuthenticated().timeout(
      ...
      // Onboarding checks only when authenticated
      final onboardingService = OnboardingService();
      final hasCompletedOnboarding = await onboardingService.isOnboardingCompleted().timeout(
        const Duration(seconds: 2),
        onTimeout: () => false,
      );
      final isFirstLaunch = await onboardingService.isFirstLaunch().timeout(
